...
public:
    static QwenBpeTokenizer LoadFromFiles(const std::string& vocab_path,
...
#if _WIN32
    static QwenBpeTokenizer LoadFromFiles(const std::wstring& vocab_path,
    ...
    QwenBpeTokenizer(QwenBpeTokenizer&& other) noexcept;
    QwenBpeTokenizer& operator=(QwenBpeTokenizer&& other) noexcept;
...
private:
    ...
    static std::vector<std::string> PretokenizeByteLevel(const std::string& text);
